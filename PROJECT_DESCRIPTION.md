...
## 12. Robustness & AI Guardrails

AI applications are inherently non-deterministic, which can lead to unpredictable application states. MetroFlow AI has been engineered with overlapping systems of safety, ensuring the application remains robust even if the underlying LLMs fail or hallucinate.

### 1. Strict Structural Guardrails (Zod Validation)
LLMs are prone to returning malformed JSON or adding conversational padding (e.g., "Here is your PRD: {...}").
- **Implementation**: We utilize the Vercel AI SDK's `generateObject` method strictly coupled with Zod schemas. The prompt explicitly forces the LLM into a constrained JSON schema. If the LLM violates the schema (e.g., outputs a string instead of an array of User Stories), the AI SDK uses structured parsing to attempt to fix it, and throws a strongly-typed error if it fails. The database is never corrupted with malformed output.

### 2. Execution Guardrails (Inngest Exponential Backoff)
LLM providers (OpenAI, Anthropic) frequently experience outages, rate limits (HTTP 429), or massive latency spikes.
- **Implementation**: Traditional serverless functions will timeout after 10-60 seconds, resulting in a dropped webhook or a spinning UI. MetroFlow routes all AI calls through Inngest. If Anthropic goes down and throws a 500 error, Inngest intercepts the failure, pauses the execution state, and automatically retries the function 3 minutes later (backing off exponentially up to 10 times). No developer PR event is ever "lost".

### 3. Logic Guardrails (The Committee of Experts)
A single AI model can suffer from "logic blindness"—where it confidently generates a flawed PRD or misses an obvious bug during code review.
- **Implementation**: As detailed in Section 9, MetroFlow uses a Multi-Model Synthesis Engine. It mitigates single-model hallucinations by generating multiple drafts concurrently across OpenAI, Gemini, and Anthropic. The master synthesizer then evaluates all drafts to form a mathematically sound consensus. If one model hallucinates, the consensus algorithm filters it out.

### 4. Data Security Guardrails (Multi-Tenancy & RBAC)
AI applications often suffer from prompt injection or cross-tenant data leakage if context is mishandled.
- **Implementation**: Every feature request, task, and review generated by the AI is strictly bound to a `workspaceId` in PostgreSQL. Before the AI is ever invoked, the tRPC router passes through an `assertWorkspaceMember` middleware. The AI is only provided context (existing features, codebase snippets) that belong explicitly to the authenticated user's workspace.
