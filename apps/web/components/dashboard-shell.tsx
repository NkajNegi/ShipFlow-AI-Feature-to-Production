...
export function DashboardShell({
  ...
  const navLink = (
    ...
    const content = (
      <>
        <Icon
          className={`h-4 w-4 shrink-0 ${isActive ? "text-[#c084fc]" : "text-muted-foreground"}`}
        />
        <span
          className={`whitespace-nowrap transition-all duration-300 ${isCollapsed ? "w-0 opacity-0 hidden" : "w-auto opacity-100 font-medium"}`}
        >
          {label}
        </span>
      </>
    ...
    return activeId ? (
      <Link
        ...
        className={`flex items-center gap-3 px-4 py-3 text-[13.5px] transition-all duration-200 group ${activeCls}`}
      >
        {content}
      </Link>
    ) : (
      <div
        title={isCollapsed ? label : undefined}
        className="flex items-center gap-3 px-4 py-3 text-[13.5px] text-muted-foreground opacity-50 cursor-not-allowed border-r-2 border-transparent"
      >
        {content}
      </div>
    );
  ...
  return (
    <div className="flex h-[100dvh] bg-[#000000] text-foreground flex-col md:flex-row overflow-hidden">
      ...
      <CommandPalette workspaceId={activeId} />
      ...
      >
        <div className="h-16 flex items-center px-4 border-b border-transparent">
          <div className="flex items-center justify-between w-full">
            <div
              className={`flex items-center gap-3 overflow-hidden transition-all duration-300 ${isCollapsed ? "w-full justify-center" : "w-auto"}`}
            >
              {/* Logo S Box */}
              <div className="w-8 h-8 shrink-0 rounded-lg flex items-center justify-center bg-gradient-to-br from-[#222] to-[#111] border border-white/10 shadow-[0_0_15px_rgba(250,204,21,0.15)]">
                <span className="text-[16px] font-extrabold text-[#facc15]">
                  S
                </span>
              </div>

              <span
                className={`font-bold text-white text-[15px] whitespace-nowrap transition-all duration-300 ${isCollapsed ? "w-0 opacity-0 hidden" : "w-auto opacity-100"}`}
              >
                MetroFlow AI
              </span>
            </div>
            ...
            {!isCollapsed && (
              <button
                ...
                className="px-1.5 py-0.5 rounded text-[10px] font-bold text-muted-foreground bg-white/5 border border-white/10 hover:text-white hover:border-white/20 transition-colors"
              >
                ⌘K
              </button>
            )}
        ...
        <nav className="flex-1 mt-4 space-y-1 overflow-y-auto scrollbar-hide">
          ...
          >
            <LayoutDashboard
              className={`h-4 w-4 shrink-0 ${pathname === `/dashboard/${activeId}` ? "text-[#c084fc]" : "text-muted-foreground"}`}
            />
            <span
              className={`whitespace-nowrap transition-all duration-300 ${isCollapsed ? "w-0 opacity-0 hidden" : "w-auto opacity-100 font-medium"}`}
            >
          ...
          {navLink("agent", "AI Copilot", Sparkles)}
          {navLink("projects", "Projects", FolderKanban)}
          {navLink("inbox", "Triage Inbox", Mail)}
          ...
          <div className="md:hidden fixed top-14 inset-x-0 bottom-0 bg-[#000000] z-50 p-4 animate-in zoom-in-95 duration-200 space-y-4 overflow-y-auto">
            ...
            <nav className="flex flex-col gap-1">
              <Link
                ...
                href={activeId ? `/dashboard/${activeId}` : "#"}
                className="flex items-center gap-3 px-3 py-3 rounded-lg text-white font-medium hover:bg-white/5"
              >
                <LayoutDashboard className="h-5 w-5 text-[#c084fc]" /> Dashboard
              </Link>
              <Link
                onClick={() => setMobileMenuOpen(false)}
                href={activeId ? `/dashboard/${activeId}/projects` : "#"}
                className="flex items-center gap-3 px-3 py-3 rounded-lg text-muted-foreground font-medium hover:bg-white/5 hover:text-white"
              >
                <FolderKanban className="h-5 w-5" /> Projects
              </Link>
              <Link
                onClick={() => setMobileMenuOpen(false)}
                href={activeId ? `/dashboard/${activeId}/inbox` : "#"}
                className="flex items-center gap-3 px-3 py-3 rounded-lg text-muted-foreground font-medium hover:bg-white/5 hover:text-white"
              >
                <Mail className="h-5 w-5" /> Triage Inbox
              </Link>
              <Link
                onClick={() => setMobileMenuOpen(false)}
                href={activeId ? `/dashboard/${activeId}/board` : "#"}
                className="flex items-center gap-3 px-3 py-3 rounded-lg text-muted-foreground font-medium hover:bg-white/5 hover:text-white"
              >
                <LayoutList className="h-5 w-5" /> Kanban Board
              </Link>
              <Link
                onClick={() => setMobileMenuOpen(false)}
                href={activeId ? `/dashboard/${activeId}/calendar` : "#"}
                className="flex items-center gap-3 px-3 py-3 rounded-lg text-muted-foreground font-medium hover:bg-white/5 hover:text-white"
              >
              ...
              <Link
                onClick={() => setMobileMenuOpen(false)}
                href={activeId ? `/dashboard/${activeId}/queue` : "#"}
                className="flex items-center gap-3 px-3 py-3 rounded-lg text-muted-foreground font-medium hover:bg-white/5 hover:text-white"
              >
                <Inbox className="h-5 w-5" /> Approval Queue
              </Link>
              <Link
                onClick={() => setMobileMenuOpen(false)}
                href={activeId ? `/dashboard/${activeId}/commits` : "#"}
                className="flex items-center gap-3 px-3 py-3 rounded-lg text-muted-foreground font-medium hover:bg-white/5 hover:text-white"
              >
                <GitCommit className="h-5 w-5" /> Commit Review
              </Link>
              <Link
                onClick={() => setMobileMenuOpen(false)}
                href={activeId ? `/dashboard/${activeId}/reviews` : "#"}
                className="flex items-center gap-3 px-3 py-3 rounded-lg text-muted-foreground font-medium hover:bg-white/5 hover:text-white"
              >
                <ShieldCheck className="h-5 w-5" /> Review History
              </Link>
              <Link
                onClick={() => setMobileMenuOpen(false)}
                href={activeId ? `/dashboard/${activeId}/analytics` : "#"}
                className="flex items-center gap-3 px-3 py-3 rounded-lg text-muted-foreground font-medium hover:bg-white/5 hover:text-white"
              >
            ...
            <nav className="flex flex-col gap-1 pb-10">
              ...
              <button
                onClick={() => setIsSigningOutModalOpen(true)}
                className="flex items-center gap-3 px-3 py-3 rounded-lg text-red-400 font-medium hover:bg-red-500/10"
              >
