  ...
  create: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      const task = await ctx.prisma.task.findUnique({
        ...
        include: { project: { select: { workspaceId: true } }, prd: { select: { featureRequest: { select: { project: { select: { workspaceId: true } } } } } } }
      });
      if (task) {
        const workspaceId = task.project?.workspaceId || task.prd?.featureRequest?.project?.workspaceId;
        if (workspaceId) await assertWorkspaceMember(ctx.prisma, ctx.session.user.id, workspaceId);
      }
      ...
      return ctx.prisma.taskComment.findMany({
        where: {
          OR: [
            ...
            { task: { prd: { featureRequest: { project: { workspaceId: input.workspaceId } } } } }
          ]
        },
        take: 10,
