...
export function resolveModel(keys: AiKeys) {
  ...
  if (process.env.OPENROUTER_API_KEY) {
    return openRouterProvider(process.env.OPENROUTER_API_KEY)(
      OPENROUTER_MODEL_ID,
    );
  }
...
/**
 * Convenience helper for the AI agent to resolve a model given a workspace/user context.
 */
export async function getWorkspaceAiModel(workspaceId: string, userId: string) {
  ...
  return resolveModel({
    ...
    anthropicUser: user?.anthropicApiKeyEnc,
    openRouterUser: user?.openRouterApiKeyEnc,
  });
}
...
export async function generateEnsembleObject<T>({
  ...
  const draft = (label: string, p: Promise<{ text: string }>) =>
    p.then((r) => r.text).catch((e) => `[${label} draft failed: ${e.message}]`);
...
/**
 * Run a structured generation through the dedicated Critic model (OpenRouter,
 * else OpenAI). Used by the codegen flow to independently audit a drafted code
 * patch before it is ever pushed to GitHub — a second, different model checking
 * the first model's work. Single-provider competitors cannot do this.
 */
export async function generateCriticObject<T>({
  keys,
  schema,
  system,
  prompt,
  ...
  const model = resolveCriticModel(keys);
  const { object } = await generateObject({ model, schema, system, prompt });
  return object;
}

export async function assertAnthropicKeyHasStrongModel(
  key: string,
): Promise<void> {
  if (!key.startsWith("sk-ant-")) {
    throw new TRPCError({
      code: "BAD_REQUEST",
      message: "Only Anthropic (Claude) API keys are allowed.",
    });
  }
  let res: Response;
  try {
    res = await fetch(
      `https://api.anthropic.com/v1/models/${STRONG_MODEL_ID}`,
      {
  ...
  throw new TRPCError({
    code: "BAD_REQUEST",
    message: "That Anthropic API key was rejected.",
  });
}
